		...
		THEN("parser can be built"){
			IfElse_T(pr, p, {
				...
				AND_THEN("what can't be parsed  - errors"){
					...
					IfOk_T(parser_parse(p, lexer0, "b", &grp_0), wast, { ast_log(wast); FAIL_CHECK("^ Parsed successfully invalid input ^"); });
					IfOk_T(parser_parse(p, lexer0, "cc", &grp_0), wast, { ast_log(wast); FAIL_CHECK("^ Parsed successfully invalid input ^"); });
					IfOk_T(parser_parse(p, lexer0, "--\n  .-.", &grp_0), wast, { ast_log(wast); FAIL_CHECK("^ Parsed successfully invalid input ^"); });
